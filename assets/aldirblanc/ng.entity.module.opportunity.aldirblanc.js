(function (angular) {
    ...
    var module = angular.module('entity.module.opportunity.aldirblanc', ['ngSanitize', 'checklist-model','infinite-scroll']);
        ...
        return {
            ...
            getGroupFieldsBySection: function () {
                ...
                fields.forEach((element, index) => {
                    //verifica se o fieldType do elemento atual é section
                    if (element.fieldType == 'section') {
                        //se for o index 0, não precisar dar o push, pq se não a primeira posição iria ficar um array vazio
                        index > 0 ? groupFieldList.push(auxGroup) : '';
                        // se o fieldType do elemento atual é 'section' então cria um array para colocar os PROXIMOS elementos dentro desse array ( ate chegar outra palavra section, ai será criado um novo array)
                        auxGroup = [];
                        auxGroup.push(element);
                    } else {
                        //os elementos subsequentes serão adicionados no array da section criado acima
                        auxGroup.push(element)
                    }
                    //SE FOR O ULTIMO INDEX, DA O PUSH TB
                    if (index === (fields.length -1)) {
                        groupFieldList.push(auxGroup)
                    }
                });

                // return fields; antes retornava isso
